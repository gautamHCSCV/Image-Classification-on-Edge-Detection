...
lr_decayed_fn = tf.keras.optimizers.schedules.CosineDecay(1e-1, 20)
callback = tf.keras.callbacks.LearningRateScheduler(lr_decayed_fn)

model.fit(train_generator, epochs=65, validation_data=validation_generator, callbacks = [callback], verbose = 1)
