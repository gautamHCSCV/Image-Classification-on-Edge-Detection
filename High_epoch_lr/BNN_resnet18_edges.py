...
lr_decayed_fn = tf.keras.optimizers.schedules.CosineDecay(2.000+1e-3, 30)
callback = tf.keras.callbacks.LearningRateScheduler(lr_decayed_fn)

model.fit(train_generator, epochs=200, validation_data=validation_generator, callbacks = [callback], verbose = 1)
