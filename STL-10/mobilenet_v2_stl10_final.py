...
out = MobileNet(input_shape=(96,96, 1), alpha=1., weights=None, classes=10)(tf.sqrt(tf.square(sobel[:,:,:,:,1])+tf.square(sobel[:,:,:,:,0])))

model = Model(inputs = inp, outputs = out)
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
print(model.summary())

lr_decayed_fn = tf.keras.optimizers.schedules.CosineDecay(1e-3, 22)
